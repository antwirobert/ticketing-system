...
export default function PendingTicketPage() {
  ...
  useEffect(() => {
    ...
    checkRefParam(); // check for ref param to update payment status
    fetchTickets();
  }, [user, navigate]);
  ...
  const handlePayment = async (ticketId: number) => {
    ...
    try {
      const response = await fetch(
        ...
        // `http://localhost:3000/ticket-system/make-payment/${ticketId}`,
        {
          method: "POST",
          headers: {
      ...
      if (json.success) {
        console.log("Payment response:", json.data.data.authorization_url);
        // toast.success("Payment successful!");
        // //*****redirect to payment platform from the response data*/
        window.location.href = json.data.data.authorization_url;
  ...
  //****function to check for ref param in the url to update an api */
  const checkRefParam = async() => {
    const url = new URL(window.location.href);
    const ref = url.searchParams.get("reference");

    if (!ref) {
      toast.warning("No reference number found in the URL.");
      return;
    }

    try {
      const response = await fetch(
        `https://nestjs.fasthosttech.com/ticket-system/update-payment/${ref}`,
        // `https://nestjs.fasthosttech.com/ticket-system/update-payment/${ref}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ ref }), // send method if your backend accepts it
        }
      );
      const json = await response.json();
      ...
      if (json.success) {
        ...
        setTickets((prevTickets) =>
          prevTickets.map((ticket) =>
            ticket.id === json.data.ticketId ? { ...ticket, status: "paid" } : ticket
          )
        );
      } else {
        toast.error(json.message || "Payment failed.");
