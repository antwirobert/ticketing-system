...
export default function PoliceGhanaCardVerificationForm() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    verifyGhanaCard(values, "police")
      .then((res) => {
        if (res.status !== 200) {
          toast.error(res.message);
        } else {
          toast.success(res.message);
          localStorage.setItem("verifiedPolice", JSON.stringify(res.user));
          navigate("/ticket", { state: res.user });
        }
